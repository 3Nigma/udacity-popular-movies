...
public class FavoriteMovieManager {
    ...
    private void changeFavoriteState(@NonNull Movie movie, boolean isItFavorited) {
        if (isItFavorited) {
            ...
            cValues.put(MoviesDatabaseHelper.TABLE_FAVORITE_MOVIES_COLUMN_ID, movie.getId());
            cValues.put(MoviesDatabaseHelper.TABLE_FAVORITE_MOVIES_COLUMN_VALUE, gson.toJson(movie));
    ...
    public List<Movie> getAllMovies() {
        ...
        Cursor fmCursor = mContentResolver.query(
                ...
                new String[] {MoviesDatabaseHelper.TABLE_FAVORITE_MOVIES_COLUMN_ID, MoviesDatabaseHelper.TABLE_FAVORITE_MOVIES_COLUMN_VALUE},
                null, null, null);
        String jMovieValue;
        Gson gson = new Gson();

        for (fmCursor.moveToFirst(); !fmCursor.isAfterLast(); fmCursor.moveToNext()) {
            jMovieValue = fmCursor.getString(fmCursor.getColumnIndex(MoviesDatabaseHelper.TABLE_FAVORITE_MOVIES_COLUMN_VALUE));
    ...
    public boolean isMovieFavorited(Movie movie) {
        Cursor fmCursor = mContentResolver.query(
                FavoriteMoviesProvider.CONTENT_URI,
                new String[] {MoviesDatabaseHelper.TABLE_FAVORITE_MOVIES_COLUMN_ID},
                MoviesDatabaseHelper.TABLE_FAVORITE_MOVIES_COLUMN_ID + " = ?",
